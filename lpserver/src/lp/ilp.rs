...
pub fn solve_ilp(
    ...
    let mut model = Model::with_env("mlu_ilp", &env)?;
    ...
    for f in 0..num_f {
        ...
        for cd in 0..num_cd {
            ...
            let var = add_binvar!(model, name: name)?;
            vs.push(var);
        }
    ...
    for e in 0..edges.len() {
        if edge_ratios.contains_key(&e) {
            let con = c!(edge_ratios[&e]
                .iter()
                .map(|&(f, ratio, f_idx, cd)| { demands[f] * ratio * action[&f_idx][cd] })
                .grb_sum()
                <= theta * caps[e]);
    ...
    // 4.5 DO NOT route via removed nodes
    if !removed_nodes.is_empty() {
        for f in 0..num_f {
            let f_idx = nc.as_ref().map_or(f, |nc| nc[f]);
            for &node in removed_nodes.iter() {
                let con = c!(action[&f_idx][node] <= 0);
                let name = &format!("con_ban{}", f);
                model.add_constr(name, con)?;
            }
        }
    }
